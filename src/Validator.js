...
export default class Validator {
  ...
  process(obj, name) {
    ...
    Object.keys(obj).forEach((key) => {
      ...
      if (typeof obj[key] === 'object') {
        // If the value contains sub-properties for the current key.
        if (
          Object.prototype.hasOwnProperty.call(obj[key], '__name') &&
          obj[key] !== undefined
        ) {
        ...
        // If the value is the validators for current key.
        ...
        rules.forEach((rule) => {
          ...
          } else throw new Error(`Conflicting Rules found for ${_name}`)
        })

        // Ensures that rules are validated in a particular order.
        validators[key].workers = Object.keys(validators[key].workers)
          .sort()
  ...
  #validate(obj, marker, response) {
    ...
    Object.keys(marker).forEach((key) => {
      ...
      const isPresent = new RuleFactory('required').rule.verify({
        value: obj[key]
      })
